...
async def post_banda(banda: BandaSchema, db: AsyncSession = Depends(get_session)):
    ...
    nova_banda = BandasModel(nome=banda.nome, 
                             qtd_integrantes=banda.qtd_integrantes, 
...
@router.get("/{bandas_id}", response_model=BandaSchema, status_code=status.HTTP_200_OK)
...
@router.put("/{bandas_id}", response_model=BandaSchema, status_code=status.HTTP_202_ACCEPTED)
...
@router.delete("/{bandas_id}", status_code=status.HTTP_204_NO_CONTENT)
